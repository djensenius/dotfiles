...
- **Reporter**: `herdr/scripts/herdr-status-report.sh` — `battery_hearts` (5 hearts) plus outdated packages as icon + count, **two entries per row** (brew, npm, pip, cargo, go, mise), pushed with `herdr workspace report-metadata`. Managers with nothing outdated are skipped and the surviving entries are repacked, so the card shrinks to as few rows as it needs rather than showing zeros. Tune with `HERDR_STATUS_PER_ROW` and `HERDR_STATUS_HEARTS`. Note it reads the per-manager `*.count` cache files directly: the plugin's own `counts-only.sh` sums brew/npm/gem/pipx only and misses cargo, go, mise and pip.
